...
The next roadmap method is available through `make search-ilqr-capture`. It runs finite-difference iLQR in dimensionless absolute-angle coordinates, simulates every transition in exact MuJoCo, tracks the resulting trajectory with time-varying feedback, and switches to LQR only after measured funnel entry. A rail-safe terminal-weight continuation on state 674 reduced Lyapunov value from `75.9 million` to `1.13 million` while keeping the planned cart excursion below `1.64 m`, but did not reach the conservative `V <= 1,800` funnel and failed after handoff. A five-second extension also failed. This is a useful full-envelope trajectory diagnostic, not a capture result or P1 evidence.
