...
`make audit-capture-pipeline` packages a stronger full-envelope teacher generator: one 4,096-sample batched 3-second predictive search, exact-MuJoCo box-constrained iLQR, a separately searched 1.5-second settling tail, strict angle/velocity handoff bounds, and an automatic 400-iteration tail fallback. Every stage is immutable and resumable. On fixed validation states 0–3 at `p=1.0`, states 0, 1, and 2 capture with continuous upright holds of at least `12.06 s` and maximum cart excursions no larger than `2.407 m`; state 3 fails after both tail stages, and a deterministic alternate seed improves its best live value from `V=14,624` to `V=3,722` without entering the `V<=1,800` funnel. A jointly optimized 4.5-second retry regresses and is rejected. This `3/4` cold-start audit is the first multi-state full-envelope model-based success, but it is below the 90% P1 requirement and is not gate evidence.
