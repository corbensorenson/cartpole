...
The next roadmap methods are available through `make search-ilqr-capture` and `make search-multiple-shooting-capture`. Box-constrained finite-difference DDP operates in dimensionless absolute-angle coordinates, simulates every transition in exact MuJoCo, tracks the resulting trajectory with time-varying feedback, and switches to LQR only after measured funnel entry. Scalar active-set constraints improve the original rail-safe trajectory from `V=1.13M` to `V=0.431M`, but that basin remains folded and energetic. Initializing from the direct-action CEM rollout and explicitly penalizing terminal dimensionless state norm produces a qualitatively better state-674 trajectory: `V=30,286`, terminal cart `0.229 m`, maximum relative-angle magnitude about `0.074 rad`, and exact agreement in uninterrupted tracking. It still misses the conservative `V <= 1,800` funnel and hits the rail after DDP ends. Sparse exact-dynamics multiple shooting is implemented with strict feasible-iterate retention and DDP tracking, but SciPy's equality solvers have not improved the exact warm start without violating defects. These are full-envelope diagnostics, not capture results or P1 evidence.
