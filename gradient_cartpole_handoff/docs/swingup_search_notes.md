...
`scripts/evaluate_ilqr_chain_basin.py` quantifies that limitation using common deterministic perturbation directions and nearest full-envelope validation states. The saved chain recovers `0/23` nearest distinct states. Around state 674 it recovers `4/32` directions at normalized radius `0.005`, `2/32` at `0.01` and `0.02`, and `0/32` at every tested radius from `0.05` through `0.8`. Scaling all DDP tracking gains by `0`, `0.05`, `0.1`, `0.25`, `0.5`, `0.75`, `1.25`, `1.5`, or `2.0` produces zero successes even at radius `0.005`; only the exact gain scale retains the four recoveries. The optimized path is therefore a very narrow feedback tube, not a broadly reusable local expert. P1 generalization must come from state-dependent replanning or robust policy learning over many replanned recoveries, not a sparse nearest-trajectory lookup.

### Hysteretic policy/LQR capture expert

The additive residual homotopy cannot provide nonlinear recovery authority when the LQR bias is saturated. `env.action_lqr_switch` is a distinct architecture: while position, absolute-angle, cart-velocity, and hinge-velocity metrics remain inside conservative exit bounds, exact LQR owns the action; outside the enter/exit hysteresis band, the neural policy owns the full normalized force. The switch never modifies MuJoCo state. Episode info and PPO episode evidence record controller mode, active state, entry/exit counts, first-entry time, and LQR/policy action-step counts.

`configs/swingup6_capture_hybrid_policy.yaml` starts the frozen-plant envelope at `p=0.0700` with `0.0025` curriculum increments. A deterministic zero-output policy establishes the honest hybrid baseline: `11/256 = 4.30%` success. In `runs/swingup6_capture_hybrid_policy_probe_100`, PPO reaches `12/256 = 4.69%` at update 50, with `100%` ever-upright rate, `58.20%` low-momentum-upright event rate, only `0.886 s` mean maximum upright streak, and `1.172 m` maximum cart excursion. It learns to avoid the rail but not to hold upright. The run was stopped after the update-50 decision gate and is not a P1 advance.

This result is consistent with the measured DDP tube and prior failed behavior cloning: small one-step action error is catastrophic when the expert is not incrementally stable. The next branch should query a state-dependent planner on learner-visited failure states and aggregate those labels on-policy, following the DAgger/FTRL direction studied in [Improved Policy Optimization for Online Imitation Learning](https://proceedings.mlr.press/v199/lavington22a.html), while explicitly measuring the stability issue emphasized by [On the Sample Complexity of Stability Constrained Imitation Learning](https://proceedings.mlr.press/v168/tu22a.html). Plain supervised fitting to nominal trajectories is not justified.
...
### On-policy DAgger rejection

`scripts/collect_capture_dagger_states.py` runs only the frozen training failures and records exact physical state, observation, learner action, LQR disagreement, source step, and Lyapunov value while the learner owns control. Round one samples eight decisions at five-step spacing from each of the first 32 hard sources. Every learner rollout fails without entering the LQR funnel. Full tanh-LQR replay from all 256 queries succeeds `0/256`; scheduled-target search on the seven step-5 queries with `V <= 5000` succeeds `0/7`.

A denser round samples steps zero through four from the same sources. Exact LQR succeeds `0/160`. The learner often lowers `V` for two steps, but the chain has already left the nonlinear recovery tube: six-generation target search on the eight lowest-V step-1 states succeeds `0/8`. High-budget feedback MPC is then tested after one learner action on sources 970 and 1196, the two states it solved at step zero. It succeeds `0/2` with a new seed pair and `0/2` with the exact successful production seeds 78405 and 78406.

The failure is not repaired by prioritizing initial decisions. The supervisor dataset now records source step, and inverse-trajectory source weights can include a `20 * exp(-step / 5)` early-decision multiplier. On the four nonlinear training sources, this reduces three initial action errors to `0.0032`, `0.0096`, and `0.0024`; the fourth remains `0.065`. Both held-out nonlinear sources remain above `0.09` or `0.06`. Nevertheless, all six closed-loop replays fail without handoff. Applying the measured small first-action errors to an otherwise exact scheduled-target teacher also makes every target rollout fail, including the `0.0024` perturbation.

The teacher tubes are therefore narrower than the direct actor's achievable error, and the learner's next states are outside all three current supervisor basins. More raw-force behavior cloning or pure-learner DAgger rounds are not justified. Subsequent capture work should retain state-dependent online replanning as the executable expert, or learn a controller representation with an explicit stability guarantee; nominal force-action MSE is no longer an acceptance metric. P1 remains unpassed.

### Hard-constrained capture and local SCP

Exact single shooting on full-envelope state 5 is rejected because its normalized terminal-constraint Jacobian reaches `9.91e10`. Aligator ProxDDP was then tested with an exact feasible FDDP warm start, hard rail/action/terminal boxes, filter acceptance, linear and nonlinear rollouts, penalty scaling, terminal homotopy, best-exact-iterate retention, and 30-step tail restriction. Nonlinear rollout removes dynamics defects, but every accepted candidate is worse than the warm start. The branch is retained as a reproducible negative result.

The useful conditioning boundary is local: the last 30 action sensitivities have maximum magnitude about `31.7`, while extending to 40 steps raises it to `1.73e8`. `scripts/search_scp_capture.py` therefore solves bounded Lyapunov-weighted least-squares subproblems on short sliding windows and accepts a control update only when an exact MuJoCo rollout reduces the normalized worst rail, terminal-box, or Lyapunov violation. Multiple window offsets provide coordinate-descent directions without reopening the full unstable horizon.

On state 5, this method first moves the terminal cart from `1.534 m` into the hard box while preserving energy, then stages practical basin thresholds down to `V=100`. The selected controller's strict replay succeeds with handoff at step 215, minimum `V=0.0102`, `12.04 s` maximum upright hold, and `1.382 m` maximum cart excursion. State 4 requires a box-first stage, energy stages down to `V=10`, and a half-width angle/velocity objective. Its selected controller does not meet that final nominal optimization target, but authoritative uninterrupted replay enters the target later, hands off at step 493, reaches `V=0.0654`, holds upright for `10.72 s`, and remains within `2.224 m`.

The selected artifacts are `runs/p1_capture_pipeline/validation_4/approach_scp_tight_velocity.json` and `runs/p1_capture_pipeline/validation_5/approach_scp_v100_focus_w45.json`. `scripts/replay_scp_capture.py` reproduces both outcomes from their controls, time-varying feedback gains, deterministic handoff settings, selected physical state, and evidence metadata. These two recoveries advance the fixed development audit to `6/6`; they are not P1 evidence because they are per-state optimized controllers over six development states rather than one feedback policy evaluated on the 1,000-state held-out envelope.
