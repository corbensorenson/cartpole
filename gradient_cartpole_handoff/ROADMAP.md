...
| P1 six-link capture basin | In progress | The seeded 20k/2k/1k envelope and strict gate evaluator are frozen. At `p=0.0700`, target planning reaches `217/256`, standard feedback MPC reaches `227/256`, and deterministic escalation reaches `233/256 = 91.02%` with a `13.90 s` median hold and no successful rail hits. The next `p=0.0725` transfer, target-replan, and standard feedback cascade reaches only `220/256 = 85.94%`, so the accepted frontier remains `p=0.0700`. On representative `p=1.0` state 674, a frozen 3-second CEM-seeded DDP approach plus a separately optimized 1.5-second settling tail and LQR fallback succeeds in uninterrupted replay: planned tail terminal `V=2,961.94`, measured funnel entry at `5.02 s`, minimum `V=0.12`, `9.30 s` upright hold, and maximum cart excursion `2.413 m`. Its measured feedback tube is extremely narrow: `0/23` nearest distinct validation states, `4/32` successes at normalized perturbation radius `0.005`, and `0/32` by radius `0.05`. Static action distillation, reward-only PPO continuation, and raw-action DAgger are rejected because one learner action leaves every measured teacher basin. Multithreaded direct-action predictive sampling gives iLQR a colder automatic full-state seed (terminal dimensionless norm `1.515`). An eight-seed archive plus independent iLQR refinements produces no latch and no success; the best uninterrupted tail reaches live `V=4,550`, while a diagnostic handoff at `V<=5,500` still fails at the rail. Candidate selection now uses actual uninterrupted replay rather than path cost, and further identical open-loop tail sampling is rejected. Feedback replanning remains the active controller branch; P1 is not passed. |
