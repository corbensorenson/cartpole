...
| P1 six-link capture basin | In progress | The seeded 20k/2k/1k envelope and strict gate evaluator are frozen. At `p=0.0700`, target planning reaches `217/256`, standard feedback MPC reaches `227/256`, and deterministic escalation reaches `233/256 = 91.02%` with a `13.90 s` median hold and no successful rail hits. The next `p=0.0725` cascade reaches only `220/256 = 85.94%`, so the accepted frontier remains `p=0.0700`. On representative `p=1.0` state 674, a frozen CEM-seeded DDP approach, settling tail, and LQR fallback succeeds in uninterrupted replay: funnel entry at `5.02 s`, minimum `V=0.12`, `9.30 s` upright hold, and maximum cart excursion `2.413 m`; its measured feedback tube is only `4/32` at normalized radius `0.005` and zero by `0.05`. Static action distillation, reward-only PPO, and raw-action DAgger are rejected. An eight-seed predictive archive reaches live `V=4,550` but never latches. Fixed-deadline receding iLQR with monotone terminal-`V` acceptance reaches `V=4,466`; additional LQR- and predictive-seeded segments fail. State-space homotopy toward the nearest held-out state fails at `alpha=0.001`; at `alpha=0.0001` it reaches transient `V=1,532` and cold terminal `V=3,648`, but strict handoff gating rejects the hot crossing and neither LQR nor a new settling tail recovers the endpoint. The verified funnel is not widened, the continuation point is not a success, and P1 is not passed. |
