...
| P1 six-link capture basin | In progress | The seeded 20k/2k/1k envelope and strict gate evaluator are frozen. At `p=0.0700`, target planning reaches `217/256`, standard feedback MPC reaches `227/256`, and deterministic escalation reaches `233/256 = 91.02%` with a `13.90 s` median hold and no successful rail hits. On representative `p=1.0` state 674, CEM-seeded box-constrained DDP reduces `V` from `75.9M` to `30,286` and reaches a physically near-upright low-momentum terminal state, but misses the `V <= 1,800` funnel and fails after the planned transient. Sparse multiple shooting has not produced a better exact-feasible path. The accepted frontier remains `p=0.0700`, far from the 1,000-state P1 gate. |
