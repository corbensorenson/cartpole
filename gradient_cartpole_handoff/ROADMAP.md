...
| P1 six-link capture basin | In progress | The seeded 20k/2k/1k envelope and strict gate evaluator are frozen. At `p=0.0700`, target planning reaches `217/256`, standard feedback MPC reaches `227/256`, and deterministic escalation reaches `233/256 = 91.02%` with a `13.90 s` median hold and no successful rail hits. On representative `p=1.0` state 674, a frozen 3-second CEM-seeded DDP approach plus a separately optimized 1.5-second settling tail and LQR fallback succeeds in uninterrupted replay: planned tail terminal `V=2,961.94`, measured funnel entry at `5.02 s`, minimum `V=0.12`, `9.30 s` upright hold, and maximum cart excursion `2.413 m`. Its measured feedback tube is extremely narrow: `0/23` nearest distinct validation states, `4/32` successes at normalized perturbation radius `0.005`, and `0/32` by radius `0.05`. A hysteretic full-authority-policy/LQR hybrid is implemented with auditable handoff metrics, but reward-only PPO at `p=0.0700` improves just `11/256` to `12/256`. Scheduled-target search solves only `4/32` sampled training failures there, so the planned on-policy supervisor must escalate through feedback MPC and DDP. The accepted curriculum frontier remains `p=0.0700`. |
