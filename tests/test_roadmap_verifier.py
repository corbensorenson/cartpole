...
class CanonicalBenchmarkTests(unittest.TestCase):
    ...
    def test_serial_angle_metric_preserves_hanging_boundary(self) -> None:
        angles = serial_absolute_angles(np.array([np.pi, 0.0, 0.0]))
        self.assertTrue(np.allclose(np.abs(angles), np.pi))
        self.assertFalse(np.any(np.isclose(np.abs(angles), 0.0)))
