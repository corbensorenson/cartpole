...
def shooting_sparsity(
    *, nx: int, horizon_steps: int, segment_steps: int, terminal_rows: int | None = None
) -> sparse.csr_matrix:
    segments = segment_count(horizon_steps, segment_steps)
    state_values = segments * nx
    terminal_rows = nx if terminal_rows is None else int(terminal_rows)
    if terminal_rows < 1:
        raise ValueError("terminal_rows must be positive")
    residual_count = segments * nx + terminal_rows + horizon_steps + horizon_steps
    ...
    pattern[terminal_start : terminal_start + terminal_rows, (segments - 1) * nx : segments * nx] = 1
    control_start = terminal_start + terminal_rows
...
def optimize_multiple_shooting(
    ...
    *,
    ...
    if terminal_factor is None:
        terminal_factor_array = np.eye(nx, dtype=np.float64)
    else:
        terminal_factor_array = np.asarray(terminal_factor, dtype=np.float64)
        if terminal_factor_array.ndim != 2 or terminal_factor_array.shape[1] != nx:
            raise ValueError("terminal factor must be a matrix with one column per state coordinate")
        if not np.all(np.isfinite(terminal_factor_array)):
            raise ValueError("terminal factor must be finite")
    ...
    def residuals(values: Array) -> Array:
        ...
        return np.r_[
            ...
            sqrt_terminal * (terminal_factor_array @ nodes[-1]),
