...
class _RolloutEvaluation:
    def __init__(
        ...
        handoff_absolute_angular_velocity_ratio: float | None = None,
        absolute_angular_velocity_scale: float = 1.0,
    ) -> None:
        ...
        self.handoff_absolute_angular_velocity_ratio = (
            None
            if handoff_absolute_angular_velocity_ratio is None
            else float(handoff_absolute_angular_velocity_ratio)
        )
        self.absolute_angular_velocity_scale = float(absolute_angular_velocity_scale)
        if self.absolute_angular_velocity_scale <= 0.0:
            raise ValueError("absolute angular velocity scale must be positive")
    ...
    def constraints(self, controls: Array) -> Array:
        ...
        absolute_angular_velocity = self.absolute_angular_velocity_scale * np.cumsum(hinge_velocity)
        ...
        if self.handoff_absolute_angular_velocity_ratio is not None:
            constraints.append(
                1.0
                - float(np.mean(absolute_angular_velocity**2))
                / self.handoff_absolute_angular_velocity_ratio**2
            )
        constraints.append(1.0 - terminal @ self.lyapunov @ terminal / self.handoff_lyapunov)
        return np.concatenate(
            [np.atleast_1d(value).astype(np.float64, copy=False) for value in constraints]
        )
    ...
    def constraint_jacobian(self, controls: Array) -> Array:
        ...
        absolute_velocity = self.absolute_angular_velocity_scale * np.cumsum(hinge_velocity)
        absolute_velocity_sensitivity = self.absolute_angular_velocity_scale * np.cumsum(
            hinge_sensitivity,
            axis=0,
        )
        ...
        if self.handoff_absolute_angular_velocity_ratio is not None:
            rows.append(
                -2.0
                * absolute_velocity
                @ absolute_velocity_sensitivity
                / (n_links * self.handoff_absolute_angular_velocity_ratio**2)
            )
        rows.append(
            -2.0
            * terminal
...
def optimize_constrained_shooting(
    ...
    *,
    ...
    evaluation = _RolloutEvaluation(
        ...
        handoff_cart_velocity_ratio,
        handoff_hinge_velocity_ratio,
        handoff_absolute_angular_velocity_ratio,
        absolute_angular_velocity_scale,
    )
