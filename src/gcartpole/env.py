...
def serial_absolute_angles(relative_angles: np.ndarray) -> np.ndarray:
    """Return wrapped absolute link angles from relative joint coordinates.

    Relative coordinates must be accumulated before wrapping.  In particular,
    the exact hanging boundary at ``+pi`` must not be pre-wrapped to ``-pi``
    before the serial sum.
    """
    return wrap_angle(np.cumsum(np.asarray(relative_angles, dtype=np.float64), axis=-1))
...
class NLinkCartPoleEnv(gym.Env):
    ...
    def _build_model(self, progress: float) -> None:
        ...
        self._cache_energy_reference()

    def _cache_energy_reference(self) -> None:
        """Cache upright/down potential levels without changing the live state."""
        ...
        self.data.qvel[:] = 0.0
        mujoco.mj_forward(self.model, self.data)
        self._upright_potential_energy = self._potential_energy()

        self.data.qpos[:] = 0.0
        self.data.qpos[1] = np.pi
        mujoco.mj_forward(self.model, self.data)
        self._down_potential_energy = self._potential_energy()
        self._energy_gap = max(
            1e-6,
            abs(self._upright_potential_energy - self._down_potential_energy),
        )
    ...
    def _energy_fraction(self, potential_energy: float | None = None) -> float:
        if potential_energy is None:
            potential_energy = self._potential_energy()
        return float(
            (float(potential_energy) - self._down_potential_energy) / self._energy_gap
        )
    ...
    def _within_lqr_switch_bounds(self, cfg: dict[str, Any], prefix: str) -> bool:
        ...
        absolute_angular_velocity = self._absolute_angular_velocity()
        ...
        absolute_limit = cfg.get(
            f"{prefix}_absolute_angular_velocity_rms",
            cfg.get("enter_absolute_angular_velocity_rms"),
        )
        if absolute_limit is not None:
            within_box = within_box and absolute_angular_velocity_rms <= float(absolute_limit)
    ...
    def _applied_action_norm(self, policy_action_norm: float) -> float:
        ...
        residual_limit = self._progress_value(residual_cfg, "residual_action_limit", 1.0)
        if residual_limit < 1.0:
            limit = abs(float(residual_limit))
            policy_action_norm = float(np.clip(policy_action_norm, -limit, limit))
        self.last_action_bias_norm = float(bias)
    ...
    def _absolute_angular_velocity(self) -> np.ndarray:
        """Return per-link angular rates, not relative joint rates."""
        return np.cumsum(np.asarray(self.data.qvel[1 : 1 + self.n], dtype=np.float64))
    ...
    def _reward(self, action_norm: float) -> float:
        ...
        energy_progress_scale = self._progress_value(reward_cfg, "energy_progress", 0.0)
        energy_level_scale = self._progress_value(reward_cfg, "energy_level", 0.0)
        ...
        reward -= float(reward_cfg.get("absolute_angular_velocity", 0.0)) * (
    ...
    def _update_upright_tracking(self) -> None:
        ...
        low_momentum_upright = bool(
            ...
            and (
                reward_cfg.get("low_momentum_streak_absolute_angular_velocity_rms") is None
                or absolute_angular_velocity_rms
                <= float(reward_cfg["low_momentum_streak_absolute_angular_velocity_rms"])
            )
        )
    ...
    def _info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "potential_energy": float(self._potential_energy()),
            "energy_fraction": float(self._energy_fraction()),
