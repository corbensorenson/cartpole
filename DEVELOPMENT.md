...
The project escalates one link at a time. The 7-link result has passed the canonical control gate, so the active target is now a reproducible **8-link hanging-start swing-up and stabilization result**. After an `n`-link result passes the same evidence bundle, the roadmap promotes the target to `n+1` until the user stops the ladder. Six-link end-to-end reproduction remains a mandatory calibration gate because six links has already been solved publicly.
...
The reviewed external training packets are summarized in [`docs/packet_review.md`](docs/packet_review.md). Their main consequence is a new global-discovery feasibility checkpoint: before scaling more capture cohorts or verification machinery, we need an exact-MuJoCo hanging-start six-link rollout that reaches upright and holds for `5 s` from feedback control. The packets' different idealized plants and benchmarks are not project evidence.

## Status

This repository contains a reproducible **canonical noisy-start seven-link
swing-up and hold result** as well as the earlier near-upright six-link
baseline. The settled-launch hybrid controller passes both final gates:
`20/20` and `100/100` noisy hanging-start episodes, with no resets and no rail
failures. It uses a 10-second hanging-equilibrium conditioning phase, a
Box-FDDP swing route, and terminal LQR capture.

The current 8-link campaign is active. Its gradient, ghost-link homotopy, and
longer-horizon FDDP branches are documented as discovery experiments; none has
yet passed the canonical 8-link 20/100 gates, so the repository makes no
8-link success claim yet.

The evidence-backed explanation is in
[`docs/seven_link_swingup_paper.md`](docs/seven_link_swingup_paper.md). The
public-claim guardrails are in
[`docs/public_release_checklist.md`](docs/public_release_checklist.md). The
canonical held-out video is
[`runs/swingup7_uniform/seven_link_swingup_success.mp4`](runs/swingup7_uniform/seven_link_swingup_success.mp4),
with machine-readable metadata beside it. The complete chain manifest is
[`runs/swingup7_uniform/seven_link_swingup_manifest.json`](runs/swingup7_uniform/seven_link_swingup_manifest.json).
The route artifact is
[`runs/swingup7_fddp_full_hanging_ilqr_terminal100k_deferred_lqr1.json`](runs/swingup7_fddp_full_hanging_ilqr_terminal100k_deferred_lqr1.json).

### Seven-Link Public Result

| Artifact | Link |
|---|---|
| Method paper | [`docs/seven_link_swingup_paper.md`](docs/seven_link_swingup_paper.md) |
| Held-out video | [`runs/swingup7_uniform/seven_link_swingup_success.mp4`](runs/swingup7_uniform/seven_link_swingup_success.mp4) |
| Policy manifest | [`runs/swingup7_uniform/seven_link_swingup_manifest.json`](runs/swingup7_uniform/seven_link_swingup_manifest.json) |
| 20-episode gate | [`runs/swingup7_uniform/eval_swingup7_20.json`](runs/swingup7_uniform/eval_swingup7_20.json) |
| 100-episode gate | [`runs/swingup7_uniform/eval_swingup7_100.json`](runs/swingup7_uniform/eval_swingup7_100.json) |

The seven-link chain passes `20/20` and `100/100` canonical noisy hanging-start
episodes. The video is a held-out noisy start, reset-free, 30 seconds, and
framed so all seven links remain visible during the upright hold.

That distinction matters. The current result is valid against this
repository's frozen canonical contract. An external record claim still
requires matching Yacine's exact plant, action frequency, timing, force limit,
rail, scoring, and submission rules.
...
This regenerates the target MuJoCo XML, records its SHA-256, checks the hanging geometry and reset distribution in MuJoCo, and runs rejection tests for easier plants and incomplete evidence. It does not claim the active eight-link control problem is solved.
...
The switch now also requires a dimensionless closed-loop Lyapunov entry score. On the frozen 512-state training audit, `V <= 2200` contains `347/347` LQR successes and no failures. All `187/187` validation states admitted by that threshold succeed when tanh-LQR remains latched; finite exit hysteresis is set from their measured worst transients. This raises the deterministic 256-state hybrid baseline to `188/256 = 73.44%` with a `13.92 s` median hold, while full tanh-LQR reaches only `193/256 = 75.39%`.

The production training-only feedback-MPC tier solves `2/8` selected hard states and accepts 86 actions. `make build-capture-supervisor-dataset` combines exact-checkpoint LQR, scheduled-target, and feedback-MPC rollouts only after each uninterrupted replay succeeds, aligns every action to its pre-action state, and emits 12,957 labels from 37 source states. Source-grouped weighted distillation reaches validation MSE `1.94e-4` but recovers none of its 37 source states in closed loop. A Lyapunov-routed PPO continuation also remains at `186/256` after 50 updates. Both static imitation and reward-only continuation are rejected; the next branch is planner labeling on learner-visited states. These are development diagnostics, not P1 evidence.

Trajectory-context distillation is implemented through `make distill-capture-trajectory`. The policy retains the same deterministic Lyapunov-gated LQR stabilizer and augments the recovery input with normalized episode time, with an optional initial-state observation. Initial-state-plus-time conditioning recovers `0/29` fitted and `0/8` source-held-out trajectories. The less overfit current-state-plus-time variant recovers `1/29` fitted and `0/8` held-out sources, then reaches `187/256 = 73.05%` on the fixed `p=0.0700` validation cohort versus the `188/256` strict-LQR hybrid baseline. Temporal context alone therefore does not rescue the 37-source dataset. The architecture remains available for a substantially broader guided-policy dataset, but this sparse-data branch is rejected.

The first genuine DAgger audit confirms that raw-force amortization is not viable for these teachers. The rejected learner generates 256 training-only queries from 32 hard sources; exact LQR recovers `0/256`. Scheduled-target replanning recovers `0/7` selected step-5 states and `0/8` selected step-1 states. Feedback MPC recovers `0/2` step-1 states even with the exact seeds that solved their initial states. Exponential early-step weighting reduces several initial action errors below `0.01`, but closed-loop recovery remains `0/6`; perturbing a successful target teacher's first action by only `0.0024` also destroys capture. Further raw-action behavior cloning is rejected. The working development controller remains online state-dependent planning with exact feedback.

The next curriculum transfer audit does not advance the accepted frontier. At `p=0.0725`, `191/256` target controllers transfer from `p=0.0700`; fresh target planning recovers 18 of 65 failures for `209/256`, and standard feedback MPC recovers 11 more for `220/256 = 85.94%`. This misses the 90% development threshold, so `p=0.0700` remains the accepted frontier. A multithreaded direct-action predictive-sampling planner now uses MuJoCo's batched rollout API, a strict same-timestep cold-handoff predicate, and LQR latching. On full-envelope state 674, a 4,096-trajectory search plus box-constrained iLQR automatically reaches terminal dimensionless state norm `1.515` in about 96 seconds of recorded planning, improving the prior successful approach's `1.776` approach norm. Reusing the batched cold-terminal sampler for the no-reset settling tail improves live minimum `V` from `8,461` to `4,550`, but still misses the verified `V<=1,800` funnel. An eight-seed predictive archive followed by independent deterministic iLQR refinements confirms that result: none latch, all eventually violate the rail, and the best uninterrupted replay again reaches `V=4,550`. Candidate selection now ranks the real uninterrupted replay before optimizer path cost, preventing a lower-cost but worse `V=7,007` tail from being selected. A diagnostic LQR switch at `V<=5,500` latches at `4.5 s` and then fails at the rail, so the wider switch is rejected. This branch is a stronger global initializer, not P1 evidence; further identical open-loop tail sampling is rejected in favor of feedback replanning.

Live feedback replanning has also been audited on state 674. Replanning predictive sampling from the broad initial state destroys the coherent long-horizon approach and reaches only live `V=1.56e6`. Preserving the approach and using a shrinking fixed-deadline receding-iLQR tail avoids terminal-horizon procrastination; a monotone terminal-`V` guard improves live minimum `V` slightly to `4,466`, but no second LQR- or predictive-seeded segment enters the verified funnel. State-space homotopy toward the nearest held-out validation state is implemented as a diagnostic. A normalized interpolation step `alpha=0.001` fails outright; `alpha=0.0001` reaches a transient `V=1,532` and a cold terminal `V=3,648`, but strict angle/cart-velocity/hinge-velocity gating correctly prevents the hot transient handoff and the cold endpoint remains outside the measured LQR basin. A separately optimized settling tail does not recover it. This establishes a very narrow trajectory-optimizer continuation frontier, not P1 evidence.

An optional [Box-FDDP](https://arxiv.org/abs/2010.00411) branch now wraps every Crocoddyl action model around the project's exact MuJoCo policy transition and analytic cost derivatives. Install it with `make setup-fddp`; run one refinement with `make search-fddp-capture`, or an acceptance-gated state homotopy with `make continue-fddp-capture`. Increasing terminal funnel and state weights by 10x resolves the earlier objective mismatch on the state-674-to-442 continuation: `alpha=0.001` reaches nominal `V=0.07`, then strict uninterrupted replay latches and holds upright for `10.56 s` within `2.402 m`. Matching the environment's `float32` policy-action boundary reduces 225-step planner/replay drift from about `1e-2` to `1.7e-8`. With that parity fix, full FDDP gains recover `32/32` perturbations through normalized radius `0.05`, `26/32` at `0.10`, and `13/32` at `0.20`; the open-loop trajectory recovers none at radius `1e-6`. Feedback continuation reaches `alpha=0.7878125`, but no tested warm start advances farther and the controller recovers `0/23` nearest distinct validation states. This is a useful local trajectory funnel, not a reusable capture policy or P1 evidence.

Successful feedback rollouts can be recentered with `make recenter-trajectory-controller`, and multiple resulting funnels can be audited with `make eval-trajectory-funnel-library`. The evaluator holds the cohort fixed, simulates every controller from every state, and labels its result as model-based oracle coverage rather than policy evidence. A library containing the root controller and ten recentered controllers succeeds on only `2/256` fixed full-envelope validation states, both with at least a 10-second hold. Repeated controller-relative recentering therefore does not expand into a reusable basin; further per-state recentering is rejected as the primary P1 strategy.

The batched predictive sampler now applies the same `float32` action quantization as the authoritative environment before every MuJoCo rollout. Repeating the representative source-trajectory plus 1.5-second receding-horizon run after this correction reproduces the prior failure: minimum `V=20,729`, no handoff, and a rail violation at `3.025 m`. The numerical mismatch is fixed, but it was not the cause of this branch's short-horizon failure. Further identical predictive sampling is rejected in favor of trajectory-conditioned or guided policy learning from successful feedback distributions.

`make audit-capture-pipeline` packages a stronger full-envelope teacher generator: one 4,096-sample batched 3-second predictive search, exact-MuJoCo box-constrained iLQR, Box-FDDP refinement, strict angle/velocity handoff bounds, and separately searched 1.5-second settling-tail fallbacks. Every stage is immutable and resumable. On fixed validation states 0–3 at `p=1.0`, all four capture with continuous upright holds of at least `10.92 s` and maximum cart excursions no larger than `2.407 m`. State 3 initially stalls at `V=14,624`; FDDP from the same primary approach reaches nominal `V=1,756.77`, then strict uninterrupted replay reaches live `V=0.11`, holds for `11.16 s`, and stays within `2.339 m`. An alternate seed also succeeds after FDDP, while a jointly optimized 4.5-second iLQR retry regresses and is rejected.

Extending that development audit through states 4–5 gives `4/6`, not `6/6`. Both failures reach an upright neighborhood but exhaust the rail before capture. State 4 reaches live `V=3,380`; a terminal-aware DDP tail extends the upright streak to `0.16 s` but still terminates at `3.013 m`. State 5 reaches live `V=1,577` after FDDP, inside the scalar funnel threshold, but fails the strict velocity-aware handoff and terminates at `3.005 m`. The runner now distinguishes the best authoritative live result from the best terminal state for extension, so a weaker tail cannot replace a stronger result or seed another tail from a degraded endpoint. This six-state cold-start audit is the first multi-state full-envelope model-based success, but it is far smaller than the 1,000-state P1 gate and is not gate evidence.

State 5 also isolates the difference between funnel proximity and usable rail margin. Its uncentered 3-second FDDP endpoint is at `x=1.534 m`, `V=1,712`, maximum angle `0.029 rad`, cart velocity `-0.247 m/s`, and hinge-velocity RMS `0.284 rad/s`; gain scaling does not change the deterministic replay, and LQR-, iLQR-, FDDP-, and predictive-sampling tails all fail. New explicit terminal cart-position and cart-velocity weights move the endpoint to `x=0.233 m` while preserving the strict angle and velocity bounds, but increase terminal `V` to `4,793`; cold FDDP and a 32-knot, 32,768-candidate predictive tail still fail from that centered endpoint. A 4.5-second rail-aware iLQR approach and an inward `1.5 m` FDDP soft boundary both regress. These branches are rejected. The next capture method must jointly produce centered terminal geometry and a genuinely recoverable internal-mode state, rather than optimizing either scalar `V` or cart position alone.

`make search-constrained-shooting-capture` audits hard rail, terminal-angle, terminal-velocity, and Lyapunov constraints with exact-MuJoCo single shooting and analytic sensitivities. On state 5, normalized constraints still produce a maximum Jacobian entry of `9.91e10`; SLSQP rejects the first subproblem as incompatible and preserves the immutable FDDP warm start. This identifies unstable long-horizon sensitivity, not constraint definition, as the numerical blocker. Further single-shooting SLSQP is rejected. The next solver branch should use constrained multiple shooting or [ProxDDP](https://simple-robotics.github.io/aligator/) so dynamic defects break that sensitivity chain while terminal and rail conditions remain explicit.

That constrained-solver audit is now implemented. `make setup-aligator` creates the pinned optional Aligator `0.19.0` environment, and `make search-proxddp-capture` converts the exact-MuJoCo Crocoddyl problem into hard-constrained ProxDDP. Exact feasible warm states, filter globalization, nonlinear rollout, penalty scaling, terminal-bound homotopy, local control boxes, and shortened tail horizons all fail to improve state 5. ProxDDP's first accepted steps violate the local control boxes through the augmented-Lagrangian iterates and leave exact terminal violations between `2.15` and `42.1`; best-exact-iterate retention correctly preserves the original controller. This branch is retained as a reproducible negative result, not a capture solution.

`make search-scp-capture` provides the successful alternative: sequential convex endpoint steps over short, sliding control windows, exact MuJoCo relinearization, bounded action updates, the dimensionless closed-loop Lyapunov metric, and exact-rollout backtracking. It monotonically centers state 5 and then uses a staged `V=1800 -> 1000 -> 500 -> 100` curriculum. The published state-5 controller strictly replays with handoff at `4.30 s`, minimum `V=0.0102`, `12.04 s` upright hold, and `1.382 m` maximum cart excursion. State 4 needs a box-first stage, the same energy curriculum, and a half-width angle/velocity target; its published controller hands off at `9.86 s`, reaches `V=0.0654`, holds for `10.72 s`, and stays within `2.224 m`. `scripts/replay_scp_capture.py` reproduces both without reset. Together with the existing successes on states 0-3, this advances the fixed six-state full-envelope development audit from `4/6` to `6/6`. It remains model-based per-state calibration over six development states, not the frozen 1,000-state P1 gate and not a reusable capture policy.

`make continue-scp-capture` runs the validated stages as a resumable, hash-checked curriculum and stops at the first authoritative success. `make audit-capture-pipeline-scp` enables that curriculum only as the final fallback in the existing predictive/iLQR/FDDP cohort runner; the default pipeline remains unchanged. A fresh out-of-calibration diagnostic at validation state 6 succeeded before the fallback: iLQR and FDDP both captured and held for `12.06 s` with `2.353 m` maximum cart excursion. This is transfer evidence for the per-state planning pipeline, not evidence for a reusable policy or the frozen P1 gate.

The published state-4 and state-5 controller SHA-256 hashes are `08d8ec774a9bf95115854c58c4b055f381a293d6c66b8e3d62b1915f07f63493` and `801cc0079170d93859ab5744ff194452117c787fe86d383d76382fda615cc7c6`, respectively.
...
The latest external review reinforces the current bottleneck: global swing-up discovery, not evidence packaging. The frontier packet produced a real three-link model-based down-start swing-up but no seven/eight-link solution; the hybrid packet improved validation methodology without producing global swing-up behavior. The new Cart-Pole Research Master Handoff adds tested mechanics, independent replay, task identity, risk accounting, and a teacher/capture research plan, but its MuJoCo gate is blocked and its seven-link fixture is a known rail-violation negative control. For this project, coarse dynamics, direct collocation, reverse curricula, saved handoff states, and teacher/learner methods are proposal tools. Exact MuJoCo replay from hanging starts decides whether they work.

The immediate discovery checkpoint is deliberately smaller than the final P3 gate: one exact uniform six-link feedback rollout must hold upright for `5 s`, followed by three additional held-out hanging-start seeds. Save full `qpos/qvel`, actions, hashes, and failure reasons. If that checkpoint fails after a declared budget, change the global discovery method instead of spending more time on verification or isolated per-state teachers.

The harmonic co-design packet adds a useful diagnostic branch: in a different idealized model, bounded mass/length/damping gradients changed modal input participation and improved small near-upright recovery counts, with the best named 7-link profile at `23/32` versus `19/32` uniform. It also exposed a lifting-work confound and failed every tested direction at `1e-3 rad`. Use its profiles and phase/energy suggestions to generate exact-MuJoCo proposals, never as canonical evidence; final evaluation restores the uniform plant, `+/-3 m` rail, `+/-80 N` force, and hanging start. See [`docs/packet_review.md`](docs/packet_review.md) for packet scope and limitations.
