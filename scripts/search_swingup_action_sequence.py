...
def state_quality(row: dict[str, Any]) -> tuple[float, float, float, float, float, float]:
    return (
        float(row.get("max_abs_angle", np.pi)),
        float(row.get("hinge_velocity_rms", np.inf)),
        float(row.get("absolute_angular_velocity_rms", np.inf)),
...
def row_from_env(env: NLinkCartPoleEnv, *, step: int, action: float, reward: float, info: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "absolute_angular_velocity_rms": float(
            np.sqrt(np.mean(env._absolute_angular_velocity() ** 2))
        ),
    ...
    *,
    ...
    for step in range(steps):
        ...
        if score_mode == "low_momentum":
            ...
            angle_over = max(0.0, float(row["max_abs_angle"]) - capture_angle_limit)
            cart_velocity_over = max(0.0, abs(float(row["cart_velocity"])) - capture_cart_velocity_limit)
            hinge_over = max(0.0, float(row["hinge_velocity_rms"]) - capture_hinge_rms_limit)
            row_score = (
                1200.0 * angle_over * angle_over
                + 180.0 * hinge_over * hinge_over
                + 120.0 * cart_velocity_over * cart_velocity_over
                + 55.0 * cart_over * cart_over
                + 8.0 * float(row["max_abs_angle"])
                + 2.0 * float(row["hinge_velocity_rms"])
                + 1.5 * abs(float(row["cart_velocity"]))
                + 0.8 * abs(float(row["x"]))
                - 8.0 * float(row["is_upright"])
                - 3.0 * float(row["capture_quality"])
                - 1.0 * float(row["max_upright_streak_seconds"])
            )
        elif score_mode == "absolute_capture_ready":
            angle_over = max(0.0, float(row["max_abs_angle"]) - capture_angle_limit)
            cart_velocity_over = max(
                0.0, abs(float(row["cart_velocity"])) - capture_cart_velocity_limit
            )
            hinge_over = max(
                0.0, float(row["hinge_velocity_rms"]) - capture_hinge_rms_limit
            )
            absolute_velocity_over = max(
                0.0,
                float(row["absolute_angular_velocity_rms"])
                - capture_absolute_velocity_limit,
            )
            row_score = (
                1200.0 * angle_over * angle_over
                + 180.0 * hinge_over * hinge_over
                + float(capture_absolute_velocity_weight) * absolute_velocity_over**2
                + 120.0 * cart_velocity_over * cart_velocity_over
                + 55.0 * cart_over * cart_over
                + 8.0 * float(row["max_abs_angle"])
                + 2.0 * float(row["hinge_velocity_rms"])
                + 2.0 * float(row["absolute_angular_velocity_rms"])
                + 1.5 * abs(float(row["cart_velocity"]))
                + 0.8 * abs(float(row["x"]))
                - 8.0 * float(row["is_upright"])
                - 3.0 * float(row["capture_quality"])
        ...
        # Handoff evidence must come from the late capture window. Without
        # this hard ranking barrier, the optimizer can report a sub-second
        # upright crossing even though the intended swing-up has not occurred.
        if score_mode in {
            "low_momentum",
            "cold_handoff",
            "capture_ready",
            "absolute_capture_ready",
        }:
...
def main() -> None:
    ...
    parser.add_argument("--capture-absolute-velocity-weight", type=float, default=800.0)
