#!/usr/bin/env python
"""Search a low-dimensional affine full-state capture controller."""
...
def affine_features(env: NLinkCartPoleEnv) -> np.ndarray:
    ...
    return np.r_[
        ...
        relative_angles / 0.15,
        absolute_rates / 0.75,
        relative_rates / 0.75,
    ].astype(np.float64)


def action(env: NLinkCartPoleEnv, vector: np.ndarray) -> float:
    features = affine_features(env)
    return float(np.tanh(features @ vector[:-1] + vector[-1]))


def main() -> None:
    parser = argparse.ArgumentParser(description="CEM search for affine relative-plus-absolute capture feedback")
    ...
    parser.add_argument("--seed", type=int, default=20781)
    ...
    for iteration in range(args.iterations + 1):
        ...
        for vector in candidates:
            metrics = rollout(
                ...
                action_fn=lambda env, actor=np.asarray(vector, dtype=np.float64): action(env, actor),
            )
        ...
        history.append(
            {
                ...
                "max_absolute_low_momentum_upright_streak_seconds": float(tm["max_absolute_low_momentum_upright_streak_seconds"]),
    ...
    vector = np.asarray(best["vector"], dtype=np.float64)
    final = rollout(
        cfg,
        state=state,
        vector=None,
        ...
        max_cart_weight=args.max_cart_weight,
        stable_time_weight=args.stable_time_weight,
        action_fn=lambda env, actor=vector: action(env, actor),
        return_trace=True,
    )
    n = int(cfg["env"]["n_links"])
    feature_names = (
        ["cart_position", "cart_velocity"]
        + [f"absolute_angle_{i}" for i in range(n)]
        + [f"relative_angle_{i}" for i in range(n)]
    ...
    payload = {
        ...
        "not_solution": True,
        "summary": "Affine full relative-plus-absolute capture CEM from an exact seven-link handoff; proposal only.",
        ...
        "best_actor": {"weight": vector[:-1].astype(float).tolist(), "bias": float(vector[-1])},
        "best_search": best,
        "eval": final,
        "search": {
            "seed": int(args.seed),
