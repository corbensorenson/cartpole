...
def swing_acceleration(
    params: np.ndarray, features: dict[str, float], t: float
) -> float:
...
def capture_blend(
    ...
    energy_gate = sigmoid(
        (float(energy_limit) - abs(float(features["energy_error"]))) / width
    )
    cart_gate = sigmoid((1.25 - cart_abs) / width)
    cart_velocity_gate = sigmoid((0.50 - cart_velocity_abs) / width)
    return float(
        np.clip(
            angle_gate * rate_gate * energy_gate * cart_gate * cart_velocity_gate,
            0.0,
            1.0,
        )
    )
...
def rollout(
    ...
    *,
    ...
    env.reset(seed=0)
    setup = setup_from_config(cfg, progress=progress)
    ...
    for step in range(steps):
        ...
        action = float(
            np.clip((1.0 - blend) * swing_action + blend * capture_action, -1.0, 1.0)
        )
        ...
        if handoff_objective == "modal":
            if upright_modes is None:
                raise ValueError("modal handoff objective requires upright modes")
            modal_metrics = modal_handoff_metrics(
                upright_modes,
            ...
            row["modal_handoff"] = modal_metrics
            cost = float(modal_metrics["score"])
        else:
            cost = (
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Search two-expert PFL swing-up plus LQR capture"
    )
    ...
    for iteration in range(args.iterations + 1):
        ...
        if iteration > 0:
            if best is not None and args.population > 1:
                candidates.append(np.asarray(best["vector"], dtype=np.float64))
            candidates.extend(
                center + rng.normal(0.0, sigma)
                for _ in range(args.population - len(candidates))
            )
        ...
        elite = np.asarray(
            [row["vector"] for row in records[: args.elites]], dtype=np.float64
        )
        ...
        history.append(
            {
                ...
                "centered_streak": float(
                    top_result["max_centered_upright_streak_seconds"]
                ),
                "low_momentum_streak": float(
                    top_result["max_low_momentum_upright_streak_seconds"]
                ),
                "best_angle": float(
                    (top_result.get("best_row") or {}).get("max_abs_angle", np.inf)
                ),
