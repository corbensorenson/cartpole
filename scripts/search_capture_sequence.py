...
def fixed_state_cfg(cfg: dict[str, Any], state: dict[str, Any], seconds: float) -> dict[str, Any]:
    env_cfg = {
        **cfg["env"],
        ...
        # A fixed-state replay must be exact.  The environment resolves
        # *_start/*_end fields before the base noise fields, so clearing only
        # init_*_noise is insufficient for curriculum configs.
        "init_cart_noise": 0.0,
