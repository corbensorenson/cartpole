...
def source_trajectory(
    ...
    # Older iLQR artifacts keep nominal states beside the controller, while
    # the chained refinement schema records them in the search block. Accept
    # both forms after applying the same shape and stitching validation.
    nominal_states = payload.get("search", {}).get("nominal_coordinate_states")
    if nominal_states is None:
        nominal_states = payload.get("controller", {}).get("nominal_coordinate_states")
    if nominal_states is None:
        raise KeyError("source artifact has no nominal_coordinate_states")
