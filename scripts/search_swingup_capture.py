...
def vector_to_controller(vec: np.ndarray, feedforward_count: int) -> dict[str, Any]:
    return {
        # A trajectory can enter the local capture basin well before the
        # legacy six-link 5-7 s window.  Keep the bound broad enough to test
        # state-gated handoffs discovered by the seven-link planner.
        "switch_time": float(np.clip(vec[0], 0.20, 12.0)),
...
def evaluate_controller(
    ...
    *,
    ...
    swing_knots = np.asarray(swing_controller["knots"], dtype=np.float64)
    ...
    for step in range(steps):
        t = step * env.dt
        swing_action = trajectory_action(env, t, swing_knots, swing_seconds, swing_kp, swing_kd)
        if t < switch_time:
            action = swing_action
        else:
...
def main() -> None:
    ...
    parser.add_argument("--swing-controller-json", default=None, help="Use a saved swing-controller search result as the pre-capture policy")
    ...
    center = np.concatenate(
        [
            np.asarray([10.80, 1.0, 0.0, 0.20, 6.0], dtype=np.float64),
            np.zeros(args.feedforward_count, dtype=np.float64),
        ]
    )
    sigma = np.concatenate(
        [
            np.asarray([0.80, 0.60, 0.55, 0.25, 1.0], dtype=np.float64),
    ...
    result = {
        ...
        "search": {
            ...
            "swing_controller_json": args.swing_controller_json,
        },
        "swing_controller": swing_controller,
        "lqr": {
